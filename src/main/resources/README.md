...
1. [설치](#설치)
2. [플러그인 기본 구조](#플러그인-기본-구조)
3. [DI 컨테이너](#di-컨테이너)
4. [명령어 @Command](#명령어-command)
5. [이벤트 @Subscribe](#이벤트-subscribe)
6. [YAML 설정](#yaml-설정)
7. [데이터베이스](#데이터베이스)
8. [GUI View](#gui-view)
9. [스케줄러](#스케줄러)
...
`CRFramework.jar`를 서버의 `plugins/` 폴더에 넣고, 내 플러그인의 `plugin.yml`에 의존성을 추가합니다.
